...
namespace BTN {
	...
	{
		this->data = n.data; this->m_height = n.m_height; this->m_color = n->m_color;
	...
	inline BinTreeNode<T>::BinTreeNode():
		parent(nullptr),left(nullptr),right(nullptr),m_height(0),m_color(RBColor::Red)
	{
	}
	...
	template<typename T>
	inline BinTreeNode<T>::BinTreeNode(T d, BNP(T) p, BNP(T) l, BNP(T) r,int h, RBColor c):
		data(d),parent(p),left(l),right(r),m_height(h),m_color(c)
	{
	}
//这里不要去重置在父亲和后代中的状态
	template<typename T>
