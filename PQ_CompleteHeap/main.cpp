// PQ_CompleteHeap.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
#include<time.h>
#include<chrono>
#include <iostream>
...
using namespace std;
void testFunc();
void testConstructor2();//测试构造2 批量构造 使用 Floyd算法
int main()
{
	testFunc();
	testConstructor2();
...
void testFunc() {
	PQ_CompleteHeap<int>pq;
	srand((unsigned int)time(NULL)/3600);
	int n = rand() % 600000; int max = -RAND_MAX;
...
void testConstructor2() {
	...
	auto tm = chrono::steady_clock::now();
PQ_CompleteHeap<int,myGreater<int> >pq(std::move(vec));
	auto dur = chrono::duration<double>(chrono::steady_clock::now() - tm);
